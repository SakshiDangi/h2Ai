...
const Upload = () => {
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (droppedFiles && droppedFiles.length > 0) {
      ...
      e.dataTransfer.clearData(); // Clear the data in the drag event
    }
  };
          ...
          <div className="space-y-6 bg-gray-700 px-6 py-6 rounded-3xl">
            ...
            <div
              className={`border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center transition-colors ${
                isDragging ? "border-health-purple bg-health-purple/10" : "border-health-gray/50 hover:border-health-purple/50"
              }`}
            ...
            >
              <svg
                className={`w-16 h-16 mb-4 ${isDragging ? "text-orage-500" : "text-white/50"}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div className="flex justify-center space-x-4">
              <label className="pill-button bg-health-purple text-white hover:bg-health-purple/90 transition-colors px-4 py-2 rounded-full flex items-center cursor-pointer">
                    ...
                    {files.map((file, index) => (
                      <li key={index} className="text-sm text-white/70 flex items-center justify-between p-2 hover:bg-health-purple/10 rounded-md">
                        <div className="flex items-center overflow-hidden">
                          <svg
                            className="w-4 h-4 mr-2 flex-shrink-0 text-health-purple"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                        ...
                        <div className="flex space-x-2 ml-2">
                          <button onClick={() => handlePreview(file)} className="text-health-purple hover:text-health-purple-light" title="Preview">
